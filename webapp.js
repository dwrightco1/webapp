...
dns.lookup('webapp-database', function(err, result) {
  console.log("dns.lookup(webapp-database) = " + result)
  database_ip = result
...
http.createServer(function(request, response) {
    ...
    con.connect((err) => {
        ...
        console.log('Wepapp Version-1.0.14');
	...
	if (typeof rows !== "undefined" && rows !== null) {
            ...
            console.log(rows);
            db_return_data = `v1.0.5: Successfully connected to database: ${database_user}@${database_ip}:${database_name}`
	}
	else {
            db_return_data = `v1.0.5: Received empty response from database: ${database_user}@${database_ip}:${database_name}`
        }
    });
