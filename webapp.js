...
http.createServer(function(request, response) {
	...
	if (typeof rows !== "undefined" && rows !== null) {
            ...
            console.log(rows);
            db_return_data = `v1.0.0: Successfully connected to database: ${database_user}@${database_ip}:${database_name}`
	}
	else {
            db_return_data = `v1.0.0: Received empty response from database: ${database_user}@${database_ip}:${database_name}`
        }
    });
